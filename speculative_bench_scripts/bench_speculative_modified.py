...
prompts = [
    ...
    "Hello? Who are you? Write code, math, and poem to explanin yourself.",
]
...
def format_as_chat(user_text: str) -> str:
        """
        Wrap a plain user message into the model-specific chat prompt.
        """
        ...
        # Fallback if the tokenizer has no template (rare).
        return user_text


def send_one_batch(base_url: str, num_prompts: int, batch_size: int):
    padded_prompts = (prompts * ((num_prompts + len(prompts) - 1) // len(prompts)))[:num_prompts]

    input_requests = []
    for raw in padded_prompts:
        chat_prompt = format_as_chat(raw)
        input_len   = len(tokenizer(chat_prompt, add_special_tokens=True)["input_ids"])
        input_requests.append(DatasetRow(chat_prompt, input_len, 512))
    ...
    results = asyncio.run(
        benchmark(
            ...
            base_url=base_url,
            model_id="default",
            tokenizer=tokenizer,          # real tokenizer here
...
def get_steps_from_speclen(spec_len):
    if spec_len >= 5:
        return 5
    return spec_len
    
def get_topk_from_speclen(spec_len):
    if spec_len >= 8:
        return 4
    return spec_len - 1
...
def main(args, server_args):
    ...
    for batch_size in args.batch_size:
        all_num_draft_tokens = spec_len_from_bs[batch_size]
        for num_draft_tokens in all_num_draft_tokens:
            steps = get_steps_from_speclen(num_draft_tokens)
            topk = get_topk_from_speclen(num_draft_tokens)
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        use_fast=False,                 # HF-fast can’t handle some chat templates yet
        padding_side="left",            # avoids left-shift of attention masks
