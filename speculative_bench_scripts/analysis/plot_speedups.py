...
def construct_result_df(data: dict, only_batch_sizes: list | None = None, baseline: str = "Autoregressive") -> tuple[pd.DataFrame, str]:
    eval_data = data["evaluation"]
    num_benchmarks = len(eval_data.keys())
    assert num_benchmarks == 1, f"Expected exactly one benchmark in the data. Found {num_benchmarks} benchmarks."
    ...
    baseline_latencies = {k: benchmark_data[baseline][k]["total_latency"] for k in benchmark_data[baseline]}
...
def plot_speedups(df: pd.DataFrame, benchmark_name: str, out_dir: str):
    ...
    for p in ax.patches:
        height = p.get_height()
        if height > 0:  # Only annotate non-zero heights (sns seems to generate some empty bars)
